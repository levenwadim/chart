class Hint{static HINT_TRIANGLE_SIZE=5;static DEFAULT_LEGEND_COLOR='#222';static DEFAULT_LEGEND_BORDER_COLOR='#fff';static ELEM_SELECTOR='.chartHint';static TRIANGLE_SELECTOR='.chartHint__triangle';static TITLE_SELECTOR='.chartHint__title';static CONTENT_SELECTOR='.chartHint__content';static ELEM_TOP_CLASS='chartHint-top';static ELEM_BOTTOM_CLASS='chartHint-bottom';static CONTENT_ROW_SELECTOR='.chartHint__content-row';static CONTENT_ROW_LEGEND_SELECTOR='.chartHint__content-row__legend';static CONTENT_ROW_VALUE_SELECTOR='.chartHint__content-row__value';constructor(parentElem){parentElem.insertAdjacentHTML('beforeend',Hint.getElementHtml());this.elem=parentElem.querySelector(Hint.ELEM_SELECTOR);this.triangle=this.elem.querySelector(Hint.TRIANGLE_SELECTOR);this.title=this.elem.querySelector(Hint.TITLE_SELECTOR);this.content=this.elem.querySelector(Hint.CONTENT_SELECTOR)}
  static getElementHtml(){return `
      ...
      `}
  static getContentRowHtml(value,index,color,borderColor){color=color||Hint.DEFAULT_LEGEND_COLOR;borderColor=borderColor||Hint.DEFAULT_LEGEND_BORDER_COLOR;return `
      ...
      `}
  getContentHtml(hoveredDot,chart){let contentHtml='';chart.data.forEach((chartData,chartIndex)=>{const linesStyle=chart.getStyle('lines',chartIndex);const pointsStyle=chart.getStyle('points',chartIndex);contentHtml+=Hint.getContentRowHtml(chartData[hoveredDot],chartIndex,linesStyle.fill,pointsStyle.fill)})
    return contentHtml}
  updContent(hoveredDot,chart){chart.data.forEach((chartData,chartIndex)=>{this.content.querySelector(Hint.CONTENT_ROW_VALUE_SELECTOR+'[data-index="'+chartIndex+'"]').textContent=chartData[hoveredDot]})}
  getHintClassName(point,chart){const defaultHintClassName=Hint.ELEM_SELECTOR.substring(1);const centerX=(chart.width-chart.padding)/2;const centerY=(chart.height-chart.padding)/2;if(point.y>centerY){return defaultHintClassName+' '+Hint.ELEM_TOP_CLASS}else{return defaultHintClassName+' '+Hint.ELEM_BOTTOM_CLASS}}
  show(hoveredDot,chart){if(this.content.innerHTML===''){this.content.innerHTML=this.getContentHtml(hoveredDot,chart)}else{this.updContent(hoveredDot,chart)}
    this.title.textContent=chart.labels[hoveredDot];const point=chart.points[hoveredDot];this.elem.className=this.getHintClassName(point,chart);if(this.elem.className.indexOf(Hint.ELEM_SELECTOR.substring(1)+'-top')!==-1){this.elem.style.top=(Math.floor(point.y-this.elem.offsetHeight-Hint.HINT_TRIANGLE_SIZE-5))+'px'}else{this.elem.style.top=(Math.floor(point.y+Hint.HINT_TRIANGLE_SIZE+5))+'px'}
    this.elem.style.left=(point.x-this.elem.offsetWidth/2)+'px';this.triangle.style.left=((this.elem.offsetWidth/2)-Hint.HINT_TRIANGLE_SIZE)+'px';this.elem.style.display='inline-block'}
  hide(){this.elem.style.display='none'}}
class Chart{static LINE_TYPE='line';DEFAULT_BG_COLOR='#222';DEFAULT_FG_COLOR='#fff';DEFAULT_PADDING=50;DEFAULT_STROKE_WIDTH=1;DEFAULT_LINE_STROKE_WIDTH=3;DEFAULT_LINE_BEZIER=!0;DEFAULT_POINT_SIZE=3;constructor(elem,data){this.params={padding:this.DEFAULT_PADDING,type:Chart.LINE_TYPE,...data,};elem.innerHTML='';this.initStyle();this.init(elem)}
